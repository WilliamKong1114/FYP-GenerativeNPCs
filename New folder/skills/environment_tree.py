...
class EnvironmentNode:
    def __init__(self, name: str, node_type: str, parent: Optional['EnvironmentNode'] = None, 
                 uuid_str: str = None, affordances: List[str] = None, game_object_name: str = None):
        self.name = name
...
class EnvironmentTree:
    ...
    def load(self):
        conn = self._get_conn()
        cur = conn.execute("SELECT uuid, name, type, parent_uuid, affordances, game_object_name FROM environment_tree")
        rows = cur.fetchall()
        conn.close()
        ...
        for r in rows:
            uid, name, ntype, pid, aff_text, gname = r
            affs = json.loads(aff_text) if aff_text else []
            node = EnvironmentNode(name, ntype, uuid_str=uid, affordances=affs, game_object_name=gname)
            self.nodes[uid] = node
            if pid:
    ...
    def save_node(self, node: EnvironmentNode):
        ...
        conn.execute("""
            INSERT OR REPLACE INTO environment_tree (uuid, name, type, parent_uuid, affordances, game_object_name)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (node.uuid, node.name, node.node_type, node.parent.uuid if node.parent else None, aff_text, node.game_object_name))
        conn.commit()
        conn.close()
        
    def add_node(self, name: str, node_type: str, parent: Optional[EnvironmentNode] = None, 
                 affordances: List[str] = None, game_object_name: str = None) -> EnvironmentNode:
        node = EnvironmentNode(name, node_type, parent, affordances=affordances, game_object_name=game_object_name)
        if parent:
    ...
    def find_suitable_location(self, action: str) -> Optional[EnvironmentNode]:
        ...
        objects = [m for m in matches if m.node_type == "object"]
        if objects:
            return objects[0]
