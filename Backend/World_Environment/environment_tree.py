...
class EnvironmentTree:
    def __init__(self, db_path: str = DB_PATH, max_depth: int = 5):
        ...
        self.routing_count_data = self.load_routing_count()
        self.usage_counters = {
            "simulation_day": None,
            "type_1": 0,
            "type_2": 0,
    ...
    def load_routing_count(self) -> Dict[str, int]:
        defaults = {"type_1": 0, "type_2": 0, "type_3": 0, "llm": 0}
        try:
            with open(self.routing_count_path, "r", encoding="utf-8") as f:
        ...
        for key, default_value in defaults.items():
            try:
                loaded[key] = int(raw_data.get(key, default_value))
            except (TypeError, ValueError):
                loaded[key] = default_value
        return loaded

    def _persist_routing_count_data(self):
        payload = {
            "time": datetime.now().isoformat(timespec="seconds"),
            "type_1": str(self.routing_count_data["type_1"]),
            "type_2": str(self.routing_count_data["type_2"]),
            "type_3": str(self.routing_count_data["type_3"]),
            "llm": str(self.routing_count_data["llm"]),
        }
        with open(self.routing_count_path, "w", encoding="utf-8") as f:
            json.dump(payload, f, indent=4)

    def _increment_route_counter(self, counter_key: str):
        with self.lock:
            self.usage_counters[counter_key] += 1
            self.routing_count_data[counter_key] += 1
            self._persist_routing_count_data()

    def refresh_usage_counters(self, simulation_day: Optional[int]):
        if simulation_day is None:
            return

        with self.lock:
            if self.usage_counters["simulation_day"] != simulation_day:
                prev_day = self.usage_counters["simulation_day"]
                if prev_day is not None:
                    print(
                        "[LOCATION][DAY SUMMARY] "
                        f"day={prev_day} "
                        f"type_1={self.usage_counters['type_1']} "
                        f"type_2={self.usage_counters['type_2']} "
                        f"type_3={self.usage_counters['type_3']} "
                        f"llm={self.usage_counters['llm']}"
                    )
                self.usage_counters = {
    ...
    def get_usage_counters(self) -> Dict[str, Optional[int]]:
        with self.lock:
            return dict(self.usage_counters)
    ...
    def find_suitable_location(self, action: str, agent_id: str, simulation_day: Optional[int] = None) -> List[EnvironmentNode]:
        if not self.root:
            self.load()
        ...
        for n in type_1:
            if re.search(re.escape(n), action, re.IGNORECASE):
                ...
                if candidates:
                    target = candidates[0]
                    self._increment_route_counter("type_1")
                    break
        ...
        if not target:
            for n in type_2:
                if re.search(re.escape(n), action, re.IGNORECASE):
                    ...
                    if candidates:
                        target = candidates[0]
                        self._increment_route_counter("type_2")
                        break
        ...
        if not target:
            for (spec_agent, keyword), loc_list in type_3.items():
                if agent_name == spec_agent and re.search(re.escape(keyword), action, re.IGNORECASE):
                    ...
                    if candidates:
                        target = candidates[0]
                        self._increment_route_counter("type_3")
                        break

        if not target and self.root:
            self._increment_route_counter("llm")
