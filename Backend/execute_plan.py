...
from observation_manager import record_observation
...
def agent_node(state: State, config: RunnableConfig):
    ...
    memory_context = ""
    if (user_msgs):    
        memory_context = memory_manager.get_memory(user_msgs[-1]["content"], user_id, clock.get_sim_hour(), partner_id)
...
def find_target(action: str, agent_id: str):
    path_nodes = tree.find_suitable_location(action, agent_id, simulation_day=clock.get_sim_days())
...
def execute_agent_action(agent_id, action, emojis, client, agent_data, cur_time, conv_manager, agent_executions):
    ...
    client.action_recorded(agent_id=agent_id, action_text=action, location=target_name, day=days, time_str=time_str, ts=time.time())
